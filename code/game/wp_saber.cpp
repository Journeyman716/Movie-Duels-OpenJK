...
void G_CreateG2HolsteredWeaponModel(gentity_t *ent, const char *psWeaponModel, int boltNum, int weaponNum, vec3_t angles, vec3_t offset)
{
	...
	if (wModelIndex)
	{
		...
		if (ent->holsterModel[weaponNum] != -1)
		{
			...
			{
				vec3_t origin = { 0, 0, 0 };
				gi.G2API_GetBoltMatrix(ent->ghoul2, ent->holsterModel[weaponNum], holsterorigin, &boltMatrix2, origin, origin, 0, NULL, ent->s.modelScale);
			}
			
			gi.G2API_AttachG2Model(&ent->ghoul2[ent->holsterModel[weaponNum]], &ent->ghoul2[ent->playerModel], boltNum, ent->playerModel);
			...
			else
			{
				...
				gi.G2API_SetBoneAnglesMatrix(&ent->ghoul2[ent->holsterModel[weaponNum]], "ModView internal default", boltMatrix2, BONE_ANGLES_PREMULT, NULL, 0, 0);
...
void G_CreateG2AttachedWeaponModel(gentity_t *ent, const char *psWeaponModel, int boltNum, int weaponNum)
{
	...
	if (wModelIndex)
	{
		...
		if (ent->weaponModel[weaponNum] != -1)
		{
			...
			gi.G2API_AttachG2Model(&ent->ghoul2[ent->weaponModel[weaponNum]], &ent->ghoul2[ent->playerModel], boltNum, ent->playerModel);
			// set up a bolt on the end so we can get where the sabre muzzle is - we can assume this is always bolt 0
...
void WP_SaberAddHolsteredG2SaberModels(gentity_t *ent, int specificSaberNum)
{
	...
	for (; saberNum <= maxSaber; saberNum++)
	{
		...
		if (saberNum == 0)
		{
			...
			{
				if ((ent->client->ps.saber[0].type == SABER_STAFF) || (ent->client->ps.saber[0].type == SABER_STAFF_SFX)
					|| (ent->client->ps.saber[0].type == SABER_STAFF_UNSTABLE) || (ent->client->ps.saber[0].type == SABER_STAFF_THIN))
				{
								...
								int saberNum, int bladeNum )
{
	...
	if ( &g_entities[tr.entityNum] )
	{
		...
		if ( attacker && attacker->client && attacker->client->ps.saberInFlight )
		{//thrown saber hit something
			if ((hitEnt && hitEnt->client && hitEnt->health > 0 && (hitEnt->client->NPC_class == CLASS_DESANN || hitEnt->client->NPC_class == CLASS_SITHLORD || hitEnt->client->NPC_class == CLASS_VADER || !Q_stricmp("Yoda", hitEnt->NPC_type) || hitEnt->client->NPC_class == CLASS_LUKE || hitEnt->client->NPC_class == CLASS_BOBAFETT || hitEnt->client->NPC_class == CLASS_MANDALORIAN || hitEnt->client->NPC_class == CLASS_JANGO || (hitEnt->client->ps.powerups[PW_GALAK_SHIELD] > 0))) ||
				 ( owner && owner->client && owner->health > 0 && ( owner->client->NPC_class == CLASS_DESANN || owner->client->NPC_class == CLASS_SITHLORD || owner->client->NPC_class == CLASS_VADER || !Q_stricmp("Yoda",owner->NPC_type) || owner->client->NPC_class == CLASS_LUKE || (owner->client->ps.powerups[PW_GALAK_SHIELD] > 0) ) ) )
					...
					if ( hitEnt != NULL )
					{
						...
						else
						{
							...
							{//only do these effects if actually trying to damage the thing...
								...
								{//no hit effect (besides regular client-side one)
									G_SaberBounce(attacker, hitEnt, qfalse);
								}
								else
								{
									if ( !WP_SaberBladeUseSecondBladeStyle( &attacker->client->ps.saber[saberNum], bladeNum )
...
void WP_SaberDamageTrace( gentity_t *ent, int saberNum, int bladeNum )
{
	...
	else
	{
		...
		{//only do once - for first blade
			...
			if ( trace.entityNum < ENTITYNUM_WORLD && (trace.entityNum > 0||ent->client->NPC_class == CLASS_DESANN || ent->client->NPC_class == CLASS_VADER || ent->client->NPC_class == CLASS_SITHLORD) )//NPCs don't push player away, unless it's Desann
			{//a valid ent
	...
	if ( ent->client->ps.saberLockTime > level.time )
	{
		...
		{//need to make some kind of effect
			...
			if ( WP_SabersIntersection( ent, &g_entities[ent->client->ps.saberLockEnemy], g_saberFlashPos ) )
			{
				...
				G_Sound(ent, G_SoundIndex(va("sound/weapons/saber/saberlock%d.wav", Q_irand(1, 9))));
			}
		}
	}
	else
...
void WP_SaberImpact( gentity_t *owner, gentity_t *saber, trace_t *trace )
{
	...
	if ( !other->takedamage && ( saber->s.eFlags & ( EF_BOUNCE | EF_BOUNCE_HALF ) ) )
	{
		...
		&& ( (other->NPC && (other->NPC->aiFlags&NPCAI_BOSS_CHARACTER))
			...
			|| other->client->NPC_class == CLASS_BOBAFETT || other->client->NPC_class == CLASS_MANDALORIAN || other->client->NPC_class == CLASS_JANGO
...
void WP_SaberStartMissileBlockCheck( gentity_t *self, usercmd_t *ucmd  )
{
	...
	for ( e = 0 ; e < numListedEntities ; e++ )
	{
		...
		{//thermal detonator!
			if ( self->NPC && dist < ent->splashRadius )
			{
				...
				else if ((self->client->NPC_class != CLASS_BOBAFETT && self->client->NPC_class != CLASS_MANDALORIAN && self->client->NPC_class != CLASS_JANGO)
					&& (self->client->NPC_class != CLASS_REBORN || self->s.weapon == WP_SABER)
	...
	if ( incoming )
	{
		if ( self->NPC && !G_ControlledByPlayer( self ) )
		{
			...
			if ((self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDALORIAN || self->client->NPC_class == CLASS_JANGO || self->client->NPC_class == CLASS_ROCKETTROOPER)
...
void WP_ForceKnockdown( gentity_t *self, gentity_t *pusher, qboolean pull, qboolean strongKnockdown, qboolean breakSaberLock )
{
	...
	if ( self->health > 0 )
	{
			...
			&& !PM_InKnockDown( &self->client->ps ) )
		{
			...
			if ( pusher->client->NPC_class == CLASS_DESANN || pusher->client->NPC_class == CLASS_SITHLORD || pusher->client->NPC_class == CLASS_VADER && self->client->NPC_class != CLASS_LUKE )
...
//FIXMEFIXMEFIXMEFIXMEFIXME: extern a lot of this common code when I have the time!!!
				...
				if ( ((self->s.number == 0 && Q_irand( 0, 2 ) ) || Q_irand( 0, 2 ) ) && push_list[x]->client && push_list[x]->health > 0 //a living client
						...
						&& ((self->client->NPC_class != CLASS_DESANN && self->client->NPC_class != CLASS_SITHLORD && self->client->NPC_class != CLASS_VADER &&Q_stricmp("Yoda",self->NPC_type)) || !Q_irand( 0, 2 ) )//only 30% chance of resisting a Desann push
				...
				else
				{
					...
					if ( pull )
					{
						...
						{//*SIGH*... if an NPC can't flee, they can't run after and pick up their weapon, do don't drop it
							...
							&& push_list[x]->client->NPC_class != CLASS_JANGO
...
void ForceJump( gentity_t *self, usercmd_t *ucmd )
{
	...
	{
	case FJ_FORWARD:
		if (((self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDALORIAN || self->client->NPC_class == CLASS_JANGO || self->client->NPC_class == CLASS_ROCKETTROOPER) && self->client->ps.forceJumpCharge > 300)
...
void ForceDestruction(gentity_t *self)
{
	...
	anim = BOTH_FORCEPUSH;
	soundIndex = G_SoundIndex("sound/weapons/force/speed.mp3");
...
qboolean PlayerAffectedByStasis(void)
{
	...
	else if (ent && ent->client && ent->client->ps.stasisJediTime > (cg.time ? cg.time : level.time))
	{
		return qtrue;
	}
	return qfalse;
	...
	if (traceEnt && traceEnt->takedamage)
	{
		...
		{//an enemy or object
			...
			{//doesn't affect jedi .but affects everything else??
				if (traceEnt->client)
				{
					traceEnt->client->ps.stasisTime = level.time + stasisTime[self->client->ps.forcePowerLevel[FP_STASIS]];
			...
			{//affect jedi.
				if (traceEnt->client)
				{
					traceEnt->client->ps.stasisJediTime = level.time + stasisJediTime[self->client->ps.forcePowerLevel[FP_STASIS]];
					VectorClear(traceEnt->client->ps.velocity);
				}
...
void ForceStasis(gentity_t *self)
{
		...
		{//I have an enemy
			...
			{//arc
				...
				{//close enough to grab
					...
					if (InFront(self->enemy->currentOrigin, self->client->renderInfo.eyePoint, self->client->ps.viewangles, minDot))
					{//need to be facing the enemy
						if (gi.inPVS(self->enemy->currentOrigin, self->client->renderInfo.eyePoint))
						{//must be in PVS
							gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, self->enemy->currentOrigin, self->s.number, MASK_SHOT, (EG2_Collision)0, 0);
	...
	else if (traceEnt->health > 0 && traceEnt->s.weapon == WP_SABER && traceEnt->client && traceEnt->client->ps.forcePower <= 75)
	{//affect jedi.
		int modPowerLevel = WP_AbsorbConversion(traceEnt, traceEnt->client->ps.forcePowerLevel[FP_ABSORB], self, FP_STASIS, self->client->ps.forcePowerLevel[FP_STASIS], forcePowerNeeded[FP_STASIS]);
		int actualPowerLevel;
		if (modPowerLevel == -1)
		...
		if (actualPowerLevel > 0)
		{
			if (traceEnt->client)
			{
				traceEnt->client->ps.stasisJediTime = level.time + stasisJediTime[actualPowerLevel];//stuck for 2-5 seconds
				VectorClear(traceEnt->client->ps.velocity);
			}
	...
	soundIndex = G_SoundIndex("sound/weapons/force/stasis.wav");
	...
	if (self->handLBolt != -1)
	{
		G_PlayEffect(G_EffectIndex("force/pushblur"), self->playerModel, self->handLBolt, self->s.number, self->currentOrigin, 200, qtrue);
	}
		...
		{//holding someone
			...
			else
			{
				...
				{//flying creature broke free
					...
					&& (drainEnt->client->NPC_class == CLASS_BOBAFETT || drainEnt->client->NPC_class == CLASS_MANDALORIAN || drainEnt->client->NPC_class == CLASS_JANGO || drainEnt->client->NPC_class == CLASS_ROCKETTROOPER)
...
void WP_InitForcePowers( gentity_t *ent )
{
	...
	{//player
		...
		{
			ent->client->ps.forcePowersKnown = (1 << FP_HEAL) | (1 << FP_LEVITATION) | (1 << FP_SPEED) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_TELEPATHY) | (1 << FP_GRIP) | (1 << FP_LIGHTNING) | (1 << FP_SABERTHROW) | (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE) | (1 << FP_RAGE) | (1 << FP_DRAIN) | (1 << FP_PROTECT) | (1 << FP_ABSORB) | (1 << FP_SEE) | (1 << FP_DESTRUCTION) | (1 << FP_STASIS);
...
void G_SaberBounce(gentity_t* self, gentity_t* other, qboolean hitBody)
{
	if (!g_saberRealisticImpact->integer)
	{
		return;
	}
