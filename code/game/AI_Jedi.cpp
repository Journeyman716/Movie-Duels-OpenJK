...
static void Jedi_AggressionErosion( int amt )
{
	...
	if (NPCInfo->stats.aggression < 4 || (NPCInfo->stats.aggression < 6 && NPC->client->NPC_class == CLASS_VADER))
	{//turn off the saber
		WP_DeactivateSaber(NPC);
	}
}
	...
	{//we're out of striking range and we are allowed to attack
		...
		else
		{
			...
			else
			{
				...
				else if ( NPC->client->NPC_class == CLASS_DESANN || !Q_stricmp("MD_Yoda",NPC->NPC_type) )
				{
					chanceScale = 1;
				}
				...
				/*else if (NPC->client->NPC_class == CLASS_YODA)
				{
					chanceScale = 1;
				}*/
				...
				{//else, randomly try some kind of attack every now and then
						...
						&& (!Q_irand( 0, 1 ) || NPC->s.weapon != WP_SABER) )
					{
						...
						else if (WP_ForcePowerUsable(NPC, FP_STASIS, 0) && Q_irand(0, 1) && NPC->enemy && NPC->enemy->client && NPC->enemy->client->ps.stasisJediTime < level.time)
						{
							ForceStasis(NPC);
							TIMER_Set(NPC, "attackDelay", NPC->client->ps.weaponTime);
						}
...
evasionType_t Jedi_SaberBlockGo( gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist = 0.0f )
{
	...
	{
		if ( self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER || self->client->NPC_class == CLASS_SITHLORD && self->s.weapon != WP_SABER)
		{
			saberBusy = qtrue;
			alwaysDodgeOrRoll = qtrue;
		}
	...
	*/
	...
	{//did some sort of evasion
		...
		{//dodged
			...
			if ( NPCInfo->rank > RANK_LT_COMM && NPC->client->NPC_class != CLASS_SITHLORD && NPC->client->NPC_class != CLASS_DESANN && NPC->client->NPC_class != CLASS_VADER)
...
void NPC_Jedi_Pain( gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod,int hitLoc )
{
	...
	{//back off
		...
		{
			self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + (3-g_spskill->integer)*50;//300
		}
		else
		{
			self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + (3-g_spskill->integer)*50;//500
		}
		if ( !Q_irand( 0, 3 ) )
...
static void Jedi_Attack( void )
{
	...
	if ( NPC->client->ps.saberLockTime > level.time )
	{
		...
		else
		{
			...
			if ( NPC->client->NPC_class == CLASS_DESANN || NPC->client->NPC_class == CLASS_SITHLORD || NPC->client->NPC_class == CLASS_VADER || !Q_stricmp("MD_Yoda",NPC->NPC_type) )
			{
	...
	{//check other special combat behavior
			...
			&& NPC->client->NPC_class != CLASS_ROCKETTROOPER )
		{
			if ( NPC->client->NPC_class == CLASS_TAVION
				|| NPC->client->NPC_class == CLASS_SHADOWTROOPER
				|| NPC->client->NPC_class == CLASS_ALORA
				|| (g_spskill->integer && ( NPC->client->NPC_class == CLASS_DESANN || NPC->client->NPC_class == CLASS_SITHLORD || NPC->client->NPC_class == CLASS_VADER || NPCInfo->rank >= Q_irand( RANK_CREWMAN, RANK_CAPTAIN ))))
