...
static void PM_GroundTraceMissed( void ) {
	...
	if ( Flying != FLY_HOVER )
	{
		if ( !(pm->ps->eFlags&EF_FORCE_DRAINED) )
		{
			...
			if (pm->ps->clientNum != 0 && pm->gent && pm->gent->NPC && pm->gent->client && pm->gent->client->NPC_class != CLASS_SITHLORD
				&& pm->gent->client->NPC_class != CLASS_DESANN && pm->gent->client->NPC_class != CLASS_VADER)//desann never falls to his death
			{
				if ( pm->ps->groundEntityNum == ENTITYNUM_NONE )
				{
					if ( pm->ps->stats[STAT_HEALTH] > 0
				...
				if ( !PM_AdjustStandAnimForSlope() )
				{
					...
					else
					{
						...
						case SS_FAST:
							...
							break;
						case SS_TAVION:
							legsAnim = BOTH_SABERTAVION_STANCE;
							break;
						case SS_STRONG:
							legsAnim = BOTH_SABERSLOW_STANCE;
							break;
						case SS_DESANN:
							legsAnim = BOTH_SABERDESANN_STANCE;
							break;
						case SS_DUAL:
				...
				{//JK2-style
					...
					if ( winAnim != BOTH_CCWCIRCLEBREAK )
					{
						if ((!genemy->s.number&&genemy->health <= 25)//player low on health
							|| (genemy->s.number&&genemy->client->NPC_class != CLASS_KYLE && genemy->client->NPC_class != CLASS_LUKE && genemy->client->NPC_class != CLASS_TAVION
								&& genemy->client->NPC_class != CLASS_ALORA && genemy->client->NPC_class != CLASS_SITHLORD && genemy->client->NPC_class != CLASS_DESANN && genemy->client->NPC_class != CLASS_VADER)//any NPC that's not a boss character
