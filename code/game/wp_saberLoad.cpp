...
qboolean WP_UseFirstValidSaberStyle(gentity_t *ent, int *saberAnimLevel)
{
	if (ent && ent->client)
	{
		...
		if (ent->client->ps.saber[0].Active() && ent->client->ps.saber[0].stylesForbidden)
		{
		...
		{//check second saber, too
			if (ent->client->ps.saber[1].Active() && ent->client->ps.saber[1].stylesForbidden)
			{
...
static keywordHash_t saberParseKeywords[] = {
	...
	{ "noClashFlare2",			Saber_ParseNoClashFlare2,		NULL	},
		// Custom saber glow, blade & dlight color code
	{ "glowshader",				Saber_ParseGlowShader,			NULL    },
	{ "bladeshader",			Saber_ParseBladeShader,			NULL    },
	{ "saberDLightColor",		Saber_ParseSaberDLightColor,	NULL    },
